...
namespace GuestBook.Controllers
{
    public class GuestbookEntryController : ApiController
    {
        ...
        public GuestbookEntryController(IGuestbookRepository repository)
        {
        ...
        public HttpResponseMessage Post(GuestbookEntry value)
        {
            if (!ModelState.IsValid)
            {
                var errors =
                    ...
                    {
                        state.Key, 
                        Errors = state.Value.Errors.Select(error => error.ErrorMessage)
                    })
            ...
            var response = Request.CreateResponse(HttpStatusCode.Created, value, Configuration);

            response.Headers.Location = new Uri(Request.RequestUri, "/api/guestbookentry/"
                + value.Id);
